...
_avg_cal:
	...
	mov [72h],00h
	mov reg[CUR_PP], 06h
	mov [D1h], [93h] 	;ram_2[10h]=value in 93h which is the sum of hours
	mov reg[CUR_PP], 00h		;set Current Page Pointer back to 0
	mov [EBh], D1h 				;initialize MVI read pointer to 10h
...
add_tonext:
	...
	mov [D2h], [92h]
	mov reg[CUR_PP], 00h	;initialize MVI read pointer to 10h
	mov [ECh], D2h
	mvi A, [ECh]		;read the value in 92h into A
	add A,[86h]
	mov [quotient_val],00h
...
add_tonext_sec:
	...
	mov [D3h], [91h]
	mov reg[CUR_PP], 00h
	mov [EDh], D3h		;initialize MVI read pointer to 10h
	mvi A, [EDh]		;read the value in 92h into A
	add A,[86h]
	mov [quotient_val],00h
